...
function generateBash(program: Command): string {
	...
	return `# bash completion for trellis/tl
...
function generateFish(program: Command): string {
	const cmds = collectCommands(program);
	const lines: string[] = ["# fish completions for trellis/tl"];

	// Disable file completions for trellis/tl
	lines.push("complete -c trellis -f");
	lines.push("complete -c tl -f");
	lines.push("");
	...
	for (const cmd of cmds) {
		lines.push(
			`complete -c trellis -n "not ${noSubcmdCond}" -a ${cmd.name} -d '${cmd.description.replace(/'/g, "\\'")}'`,
		);
		lines.push(
			`complete -c tl -n "not ${noSubcmdCond}" -a ${cmd.name} -d '${cmd.description.replace(/'/g, "\\'")}'`,
		);
	}
	...
	for (const cmd of cmds) {
		for (const sub of cmd.subcommands) {
			lines.push(
				`complete -c trellis -n "__fish_seen_subcommand_from ${cmd.name}" -a ${sub.name} -d '${sub.description.replace(/'/g, "\\'")}'`,
			);
			lines.push(
				`complete -c tl -n "__fish_seen_subcommand_from ${cmd.name}" -a ${sub.name} -d '${sub.description.replace(/'/g, "\\'")}'`,
			);
		}
		for (const o of cmd.options) {
			...
			lines.push(
				`complete -c trellis -n "__fish_seen_subcommand_from ${cmd.name}" ${longFlag} -d '${o.description.replace(/'/g, "\\'")}'`,
			);
			lines.push(
				`complete -c tl -n "__fish_seen_subcommand_from ${cmd.name}" ${longFlag} -d '${o.description.replace(/'/g, "\\'")}'`,
			);
		}
	}
...
/**
 * Register the 'completions' command with the given program.
 */
